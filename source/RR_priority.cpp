...
void rr_priority_scheduling(vector<Process> processes) {
    ...
    int time_quantum = 5; // RR 時間量子

    // 每個優先級維護一個 FIFO 佇列，實現 RR within Priority
    vector<queue<int>> priority_queues(4); // 優先級 1, 2, 3 (索引 1, 2, 3)
    ...
    while (completed_processes < num_processes) {
        ...
        for (int i = 0; i < num_processes; ++i) {
            if (processes[i].arrival_time <= current_time && processes[i].remaining_burst_time > 0 && processes[i].last_run_time <= current_time) {
                ...
                for(int p_level = 1; p_level <= 3; ++p_level) {
                    queue<int> temp_q = priority_queues[p_level]; // 複製以遍歷
                    while(!temp_q.empty()){
        ...
        for (int p_level = 1; p_level <= 3; ++p_level) {
            if (!priority_queues[p_level].empty()) {
                ...
                break; // 找到最高優先級的行程，跳出循環
            }
        }

        if (current_proc_id == -1) { // 如果所有佇列都空，CPU空閒
        ...
        // 執行一個時間量子或直到完成 (非搶佔式優先級，但 RR 內部搶佔)
    ...
    print_gantt_chart(gantt_chart_data); // 列印甘特圖
    print_results(original_processes, context_switches, "RR + Non-Preemptive Priority Scheduling"); // 列印結果
}